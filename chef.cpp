...
Chef::Chef(const unsigned int id) : id(id), atendimento(nullptr) {
    nomeArquivo = "ChefeCozinha_" + std::to_string(id) + ".txt";
    log.open(nomeArquivo, std::ios::out);
    if (log.is_open()) {
        log << "ChefeCozinha_" << id << "\n\n";
    }
    log.close();
}
...
void Chef::iniciarAtendimento(const unsigned int mesa) {
    if (atendimento != nullptr) {
        std::cerr << "Chefe " << id << " já está atendendo uma mesa!" << std::endl;
        return;
    }
    mesaAtual = mesa;
    registrarInicioMesa(mesa);
...
void Chef::prepararPedido(const std::string &pedido) {
    if (atendimento == nullptr) {
        std::cerr << "Nenhum atendimento iniciado para o chef " << id << "!" << std::endl;
...
bool Chef::estaLivre() const {
    return atendimento == nullptr;
}
void Chef::registrarInicioMesa(unsigned int mesa) {
...
void Chef::registrarEncerramentoMesa() {
    log.open(nomeArquivo, std::ios::app);
    if (log.is_open()) {
        log << "\n"; // linha em branco para separar mesas
        log.close();
    }
}
